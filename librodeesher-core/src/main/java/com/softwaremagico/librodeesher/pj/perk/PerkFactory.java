...
public class PerkFactory {
		...
		while (index < lines.size() && !lines.get(index).equals("")
			...
			if (line.length() > 2) { // No end of lines.
				try {
					...
					PerkCategory classification = PerkCategory
							.getPerkCategory(descomposed_line[3]);
					String description = descomposed_line[5];
					Perk perk = new Perk(perkName, cost, classification,
					...
					ShowMessage.showErrorMessage("Error en el talento:\""
							+ line + "\"", "Talentos");
				}
			}
			index++;
	...
	private static void addListToChooseBonus(Perk perk, String optionsLine) {
		...
		if (set[0].toLowerCase().contains(Spanish.ANY_CATEGORY)) {
			...
			perk.addSkillsToChoose(new ChooseSkillGroup(options, SkillFactory
					.getWeaponSkills(), getOption(bonusString)));
	...
	private static void addDefinedBonus(Perk perk, String optionsLine) {
		...
		if (CategoryFactory.existCategory(bonusName)) {
			if (bonusString.toLowerCase().contains(Spanish.COMMON_TAG)) {
				...
				Integer bonusNumber = Integer.parseInt(bonusString.replace("*",
						"").replace("r", ""));
				if (conditionalBonus) {
			...
			ShowMessage.showErrorMessage(
					"Bonus " + bonusName + " no reconocido en el talento:\""
	...
	private static void addBonuses(Perk perk, String bonusesDescription) {
		...
		try {
			...
			ShowMessage.showErrorMessage("Error en el bonus del talento:\""
					+ perk.getName() + "\"", "Talentos");
		}
	}
