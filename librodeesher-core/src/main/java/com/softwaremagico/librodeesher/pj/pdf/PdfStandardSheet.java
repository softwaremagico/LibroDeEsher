...
public class PdfStandardSheet {
	...
	private void whitePage() throws Exception {
		String font = FontFactory.HELVETICA;
	...
	private void addSpecializedSkillLine(Skill hab, String font, int fontSize,
			PdfPTable table, int i, int specializedIndex)
	...
	private void addEmptySkillLine(String font, int fontSize,
	...
	private void skillPage(Document document, PdfWriter writer,
	...
	private PdfPTable createMainHeader(String font,
			int fontSize) {
		PdfPCell cell;
		Paragraph p;
	...
	private PdfPTable createMainPageLeftFrame(String font,
			int fontSize) {
		PdfPCell cell;
	...
	private PdfPTable createRuneTable(String font,
			int fontSize) {
	...
	private PdfPTable createPointsTable(String font, int fontSize) {
		...
		tabla.addCell(cell);
		if (characterPlayer != null) {
			p = new Paragraph(characterPlayer.getPowerPoints() + "",
					FontFactory.getFont(font, fontSize + 3));
		} else {
			p = new Paragraph("", FontFactory.getFont(font, fontSize + 3));
		}
	...
	private PdfPTable createMainPageRightFrame(String font,
			int fontSize) {
		PdfPCell cell;
		Paragraph p;
	...
	private void addMainTable(Document document, PdfWriter writer,
	...
	private void addEmptyText(Document document, PdfWriter writer,
			String font, int fontSize) {
		PdfPCell cell;
		Paragraph p;
	...
	protected Image getNewRanksImage(int ranks)
			throws BadElementException, MalformedURLException, IOException {
		Image image;
		switch (ranks) {
		case 1:
