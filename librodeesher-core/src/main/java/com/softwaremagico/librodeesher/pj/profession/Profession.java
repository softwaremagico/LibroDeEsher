...
public class Profession {
	...
	private HashMap<String, Integer> trainingCostsNotMagic;
		...
		while (!lines.get(index).equals("") && !lines.get(index).startsWith("#")) {
			...
			for (int i = 0; i < skillColumns.length; i++) {
				...
				if (skillColumns[i].contains("#")) {
					...
					Category cat = CategoryFactory.getCategory(categoryColumns[0].replace("{", "").replace(
							"}", ""));
					if (cat != null) {
					...
					} else if (categoryColumns[0].contains("{") && categoryColumns[0].contains("}")) {
						String[] skillsArray = null;
						// One skill from a list.
						if (categoryColumns[0].contains(";")) {
							skillsArray = categoryColumns[0].replace("{", "").replace("}", "").split(";");
						} else {
							// One skill that has a prefix.
							List<String> skills = SkillFactory.getSkillNames(SkillFactory
									.getSkills(categoryColumns[0].replace("{", "").replace("}", "")));
							skillsArray = new String[skills.size()];
							skillsArray = skills.toArray(skillsArray);
						}
						try {
							ChooseSkillGroup chooseSkills = new ChooseSkillGroup(
									Integer.parseInt(categoryColumns[1]), skillsArray, chooseType);
							groupSkillsToChoose.add(chooseSkills);
	...
	private int setTrainingCosts(List<String> lines, int index) throws InvalidProfessionException {
		...
		trainingCostsNotMagic = new HashMap<>();
		...
		while (index < lines.size() && (!lines.get(index).startsWith("#"))) {
			if (lines.get(index).length() > 0) {
				...
				try {
					...
					// Not magic campains
					if (trainingColumns.length > 2) {
						Integer costNotMagic = Integer.parseInt(trainingColumns[2].replace("+", "").replace("-", "")
								.trim());
						trainingCostsNotMagic.put(trainingName, costNotMagic);
					}
				} catch (Exception e) {
	...
	public Integer getTrainingCostNotMagic(String trainingName) {
		Integer cost = trainingCostsNotMagic.get(trainingName);
		if (cost != null) {
			return cost;
		}
		return getTrainingCost(trainingName);
	}
