...
public class Profession {
	...
	public boolean isCharacteristicProfessional(
			CharacteristicsAbbreviature characteristicAbbreviature) {
		if (characteristicPreferences.size() > 0
				&& characteristicAbbreviature.equals(characteristicPreferences
						.get(0))) {
			return true;
		}
	...
	private void readProfessionFile(String professionName)
			throws InvalidProfessionException {
		int lineIndex = 0;

		String professionFile = RolemasterFolderStructure
				.getDirectoryModule(ProfessionFactory.PROFESSION_FOLDER
		...
		if (professionFile.length() > 0) {
			...
			lineIndex = setSpecialSkills(lines, lineIndex, commonSkills,
					commonSkillsToChoose, ChooseType.COMMON);
			lineIndex = setSpecialSkills(lines, lineIndex, professionalSkills,
					professionalSkillsToChoose, ChooseType.PROFESSIONAL);
			lineIndex = setSpecialSkills(lines, lineIndex, restrictedSkills,
					restrictedSkillsToChoose, ChooseType.RESTRICTED);
			...
			throw new InvalidProfessionException("Invalid profession file: "
					+ ProfessionFactory.PROFESSION_FOLDER + File.separator
					+ professionName + ".txt");
		}
	}

	public CategoryCost getMagicCost(MagicListType listType,
			Integer currentListranks) {
		return magicCosts.getListCost(listType,
				MagicLevelRange.getLevelRange(currentListranks + 1));
	}

	public CategoryCost getMagicCost(MagicListType listType,
			MagicLevelRange levelRange) {
	...
	private int setBasicCharacteristics(List<String> lines, int index)
		...
		if (!lines.get(index).toLowerCase().contains("indiferente")) {
			String[] characteristicsTags = preferenciasCaracteristicas
					.split(" ");
			...
			for (String abbrev : characteristicsTags) {
				if (Characteristics.isCharacteristicValid(abbrev)) {
					...
					throw new InvalidProfessionException("Caracteristica "
							+ abbrev + " mostrada en el archivo " + name
							+ ".txt no existente.");
				}
			}
		}
		...
		while (!lines.get(index).equals("")
			...
			try {
				if (categoryName.startsWith("Armas·")) {
					CategoryCost weaponsCost = new CategoryCost(
							categoryColumns[1]);
					weaponCategoryCost.add(weaponsCost);
					weaponsCost.setCategoryCostId(CategoryGroup.WEAPON
							.toString() + weaponCategoryCost.size());
				} else {
					Category cat = CategoryFactory.getCategory(categoryName);
					categoryCost.put(cat.getName(), new CategoryCost(
							categoryColumns[1]));
				}
			} catch (Exception e) {
	...
	private void createExtraWeaponsCosts() {
		...
		for (int i = 0; i < HOW_MANY_EXTRA_COSTS; i++) {
			CategoryCost weaponsCost = new CategoryCost(weaponCategoryCost.get(
					weaponCategoryCost.size() - 1).getRankCost());
			extraWeaponCategoryCost.add(weaponsCost);
			weaponsCost.setCategoryCostId(CategoryGroup.WEAPON.toString()
					+ (weaponCategoryCost.size() + extraWeaponCategoryCost
							.size()));
		}
	}
	...
	private int setSpecialSkills(List<String> lines, int index,
		...
		while (!lines.get(index).equals("")
			...
			if (skillLine.toLowerCase().contains("ninguna")
			...
			for (int i = 0; i < skillColumns.length; i++) {
				...
				if (skillColumns[i].contains("#")) {
					...
					Category cat = CategoryFactory
							.getCategory(categoryColumns[0]);
					if (cat != null) {
						ChooseSkillGroup chooseSkills = new ChooseSkillGroup(
								Integer.parseInt(categoryColumns[1]),
								cat.getSkills(), chooseType);
						groupSkillsToChoose.add(chooseSkills);
					} else {
						throw new InvalidProfessionException(
								"Error leyendo una categoría en habilidad común: "
										+ lines.get(index));
					}
					// One skill of a set
				} else if (skillColumns[i].startsWith("{")) {
					String skillGroup = skillColumns[i].replace("{", "")
							.replace("}", "");
					ChooseSkillGroup chooseSkills;
					try {
						chooseSkills = new ChooseSkillGroup(1, skillGroup
								.replace(";", ",").split(", "), chooseType);
						groupSkillsToChoose.add(chooseSkills);
	...
	private int setTrainingCosts(List<String> lines, int index)
		...
		while (!lines.get(index).equals("")) {
			while (!lines.get(index).equals("")) {
				...
				try {
					Integer cost = Integer.parseInt(trainingColumns[1]
	...
	public class CategoryCostComparator implements Comparator<CategoryCost> {
		...
		public int compare(CategoryCost o1, CategoryCost o2) {
			if (o1.getMaxRanksPerLevel() > o2.getMaxRanksPerLevel()) {
				...
				return 1;
			} else if (o1.getMaxRanksPerLevel() > 1
					&& o1.getRankCost(1) < o2.getRankCost(1)) {
				return -1;
			} else if (o1.getMaxRanksPerLevel() > 1
					&& o1.getRankCost(1) > o2.getRankCost(1)) {
				return 1;
			} else if (o1.getMaxRanksPerLevel() > 2
					&& o1.getRankCost(2) < o2.getRankCost(2)) {
				return -1;
			} else if (o1.getMaxRanksPerLevel() > 2
					&& o1.getRankCost(2) > o2.getRankCost(2)) {
				return 1;
