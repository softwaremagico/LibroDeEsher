...
public class Log {
    ...
    static {
        try {
            ...
            FileHandler fh = new FileHandler(homeUsuario + File.separator + "LibroDeEsher.log", maxBytes, numLogFiles, true);
            logger.addHandler(fh);
            logger.setLevel(logLevel);
            //fh.setFormatter(new SimpleFormatter());
            fh.setFormatter(getCustomFormatter());
        } catch (IOException | SecurityException ex) {
            System.out.println("Logger failed. Probably the log file can not be created. Error Message: " + ex.getMessage());
        }
    }
